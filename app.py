...
def get_db_connection():
    try:
        ...
        app.logger.error(f"連線池錯誤，使用直接連線: {e}")
...
def create_quick_reply_buttons():
    buttons = [
        QuickReplyButton(action=MessageAction(label="結算", text="結算")),
        QuickReplyButton(action=MessageAction(label="說明", text="說明")),
        QuickReplyButton(action=MessageAction(label="移除最後一筆", text="移除最後一筆"))
    ]
    ...
    try:
        ...
        for user in users:
            ...
            buttons.append(
                QuickReplyButton(
                    action=MessageAction(label=f"{user_name} 結算", text=f"{user_name} 結算")
                )
            )
    except Exception as e:
...
def handle_message(event):
    ...
    if msg == "我愛你":
        line_bot_api.reply_message(
            event.reply_token, 
            TextSendMessage(text="我也愛你們爸爸媽媽 ❤️", quick_reply=create_quick_reply_buttons())
        )
        return
    ...
    if msg.startswith("我是"):
        ...
        if name:
            ...
            try:
                ...
                line_bot_api.reply_message(
                    event.reply_token, 
                    TextSendMessage(text=f"✅ 歡迎 {name}！已記住你的名字。", quick_reply=create_quick_reply_buttons())
                )
            except Exception as e:
                app.logger.error(f"設定名字失敗: {e}")
                line_bot_api.reply_message(
                    event.reply_token, 
                    TextSendMessage(text="❌ 設定失敗，請稍後再試。", quick_reply=create_quick_reply_buttons())
                )
            finally:
                if conn:
                    return_db_connection(conn)
    ...
    if msg == "移除最後一筆":
        ...
        try:
            ...
            line_bot_api.reply_message(
                event.reply_token,
                TextSendMessage(text="❌ 移除失敗，請稍後再試。", quick_reply=create_quick_reply_buttons())
            )
        finally:
            if conn:
                return_db_connection(conn)
    ...
    if match_explicit or match_implicit:
        ...
        if date_str:
            try:
                ...
                line_bot_api.reply_message(
                    event.reply_token, 
                    TextSendMessage(text="❌ 日期格式請用 YYYY-MM-DD", quick_reply=create_quick_reply_buttons())
                )
                return
        ...
        try:
            ...
            line_bot_api.reply_message(
                event.reply_token, 
                TextSendMessage(text="❌ 拆帳失敗，資料庫錯誤", quick_reply=create_quick_reply_buttons())
            )
        finally:
            if conn:
                return_db_connection(conn)
    ...
    if match:
        ...
        try:
            ...
            line_bot_api.reply_message(
                event.reply_token, 
                TextSendMessage(text="❌ 記帳失敗，資料庫出錯了。", quick_reply=create_quick_reply_buttons())
            )
        finally:
            if conn:
                return_db_connection(conn)
    ...
    if match_settle:
        ...
        try:
            ...
            if specific_name:
                ...
                if not target_uid:
                     line_bot_api.reply_message(
                         event.reply_token, 
                         TextSendMessage(
                             text=f"❌ 找不到「{specific_name}」！請確認名字有註冊過。",
                             quick_reply=create_quick_reply_buttons()
                         )
                     )
                     return
                ...
                if debts:
                    ...
                    if specific_name in debts:
                        for creditor, amount in debts[specific_name].items():
                            personal_debts.append(f"  欠 {creditor}: ${amount}")
                    # 查看是否別人欠自己錢
                    for debtor, creditors in debts.items():
                        if specific_name in creditors:
                            personal_debts.append(f"  {debtor} 欠你: ${creditors[specific_name]}")
            ...
            line_bot_api.reply_message(
                event.reply_token, 
                TextSendMessage(text="❌ 結算發生錯誤", quick_reply=create_quick_reply_buttons())
            )
        finally:
            if conn:
                return_db_connection(conn)
    ...
    if msg == "清除":
        ...
        try:
            ...
            line_bot_api.reply_message(
                event.reply_token,
                TextSendMessage(text="❌ 清除失敗，請稍後再試。", quick_reply=create_quick_reply_buttons())
            )
        finally:
            if conn:
                return_db_connection(conn)
